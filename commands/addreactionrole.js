...
exports.run = async (client, msg, args) => {
  ...
  if (!msgs.size)
    return msg.channel.send(
      "Did not get a valid response within 30 seconds, cancelling."
    );
  if (msgs.first().content <= 0)
    return msg.channel.send("Integer must be larged than 0.");
  if (msgs.first().content > 20)
    return msg.channel.send("Integer was larger than 20, cancelling.");
  ...
  while (value > 0) {
    ...
    if (roles.includes(msgs.first().content))
      return msg.channel.send(
        "You already had that role added onto a message, remove it before adding it again."
      );

    msg.channel.send(`Now give me the emoji **name** or **id**.`);
    const emojiInput = await msg.channel.awaitMessages(
      res => res.author.id === msg.author.id,
      {
        max: 1,
        time: 30000
    ...
    let emoji =
      client.emojis.get(emojiInput.first().content) ||
      client.emojis.find(e => e.name === emojiInput.first().content);
    if (!emoji)
      return msg.channel.send("Could not find that emoji, returning.");

    client.reactionroles.push(
      msg.guild.id,
      { emoji: emoji.name, emojiid: emoji.id, role: msgs.first().content },
      "roles"
    );
    value--;
  ...
  let embed = new Discord.MessageEmbed()
    ...
    .setDescription(
      array2.map(
        i =>
          `${client.emojis.get(i.id)} ➜ ${msg.guild.roles.get(
            msg.guild.roles.find(r => r.name === i.role).id
          )}\n`
      )
    )
    .setColor("#28bcec");
